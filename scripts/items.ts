...
const items = readJsonSync("json/items.json")
  .items.item.filter((item) => {
    ...
    return !array(item.property).find(
      (p) => p._name === "CreativeMode" && p._value === "None",
    )
  })
  .map((item) => item._name)
  .filter((name) => !name.includes("Admin"))

const mods = readJsonSync(
  "json/item_modifiers.json",
).item_modifiers.item_modifier.map((mod) => mod._name)

const recipes = readJsonSync("json/recipes.json").recipes.recipe.map(
  (recipe) => recipe._name,
)

const pickup = readJsonSync("json/blocks.json")
  .blocks.block.filter((block) => block.property)
  .filter((block) => {
    return array(block.property).find(
      (p) => p._name === "CanPickup" && p._value === "true" && !p._param1,
    )
  })
...
readJsonSync("json/blocks.json")
  .blocks.block.filter((block) => block.drop)
  .forEach((block) => {
    array(block.drop)
...
const all = uniq([...items, ...mods, ...recipes, ...pickup, ...harvest])
