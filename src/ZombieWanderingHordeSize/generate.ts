...
json.gamestages.spawner
  .find((s) => s._name === "WanderingHorde")
  .gamestage.forEach((gs) => {
...
const result = gamestages
  .map((gs) => {
    ...
    return `<set xpath='/gamestages/spawner[@name="WanderingHorde"]/gamestage[@stage="${
      gs.stage
    }"]/spawn[@group=${group} and @num="${gs.num}"]/@num'>${
      +gs.num * factor
    }</set>`
  })
  .join("\n")
