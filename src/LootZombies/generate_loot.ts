...
function renderLootcontainer(name, index, items) {
  return `
    <!-- ${name} -->
    <lootcontainer id="${BAG_ID_OFFSET + index}" count="${ITEMS_PER_BAG}" size="${BAG_SIZE}" destroy_on_close="false" sound_open="UseActions/open_backpack" sound_close="UseActions/close_backpack" open_time="1" loot_quality_template="baseTemplate">
${items}
    </lootcontainer>`
}
...
uniq(missing).sort().forEach(name => {
