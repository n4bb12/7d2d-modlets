...
const deletions = dyes
  ...
  .map(
    (c) =>
      `  <remove xpath='/item_modifiers/item_modifier[@name="modDye${upperFirst(
        c.name,
      )}"]'/>`,
  )
  .join("\n")
...
const insertions = dyes
  .map(
    (dye) => `
    <!-- ${dye.name} -->
    <item_modifier name="modDye${upperFirst(
      dye.name,
    )}" installable_tags="clothing,armor,weapon,tool,vehicle" modifier_tags="dye" type="attachment" rarity="0.2">
